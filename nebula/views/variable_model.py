...
from nebula_meta.variable_model import VariableModel, add_variable_to_registry, get_all_variables

logger = logging.getLogger('nebula.api.variable_model')
...
class VariableModelListHandler(BaseHandler):
    REST_URL = "/platform/variable_models"
    ...
    def get(self):
        ...
        modules = self.get_arguments("modules") or list()
        simple = self.get_argument('simple', default="false")
        simple = simple == 'true'

        self.set_header('content-type', 'application/json')
        try:
            result = VariableModelCustDao().list_all_models()
            if app:
            ...
            # modules为module列表
            if modules:
                result = filter(lambda model: model.module in modules, result)
    ...
    def delete(self):
        ...
        self.set_header('content-type', 'application/json')
        try:
            VariableModelCustDao().delete_model_list_by_app_type_module(app, type, module)
...
class VariableModelQueryHandler(BaseHandler):
    REST_URL = "/platform/variable_models/variable/{app}/{name}"
    ...
    def get(self, app, name):
        ...
        self.set_header('content-type', 'application/json')
        try:
            result = VariableModelCustDao().get_model_by_app_name(app, name)
            if result:
                if simple:
...
class VariableModelBeautifyHandler(BaseHandler):
    """
    对variable进行美化输出，不进行存储
    """

    REST_URL = "/platform/variable_models_beautify"
